...
function updateMap(data) {
    ...
    establishments.forEach((est, index) => {
        const estIcon = L.divIcon({
            className: 'custom-div-icon',
            html: `<div style="background-color: #8B5CF6; width: 16px; height: 16px; border: 2px solid white; border-radius: 50%; box-shadow: 0 0 8px rgba(139, 92, 246, 0.8); display: flex; align-items: center; justify-content: center; color: white; font-size: 8px; font-weight: bold;">${index + 1}</div>`,
            iconSize: [16, 16],
            iconAnchor: [8, 8]
        });

        const estMarker = L.marker([est.lat, est.lng], { icon: estIcon })
            .bindPopup(`<b>${est.name}</b><br>${est.address}<br><i>${est.description}</i>`)
            .addTo(map);
