...
app.post('/api/calculate', async (req, res) => {
    ...
    if (!midpoint) {
        return res.status(400).json({ error: 'Dados do ponto médio estão ausentes.' });
    }
...
Encontre e recomende entre 3 e 4 estabelecimentos comerciais reais excelentes e bem avaliados (especialmente do tipo "${selectedPlaceType || 'cafes, restaurantes'}") que fiquem no bairro "${midpoint.neighborhood}" na cidade "${midpoint.city}", ou bem próximos à coordenada do ponto médio (Lat: ${midpoint.lat}, Lng: ${midpoint.lng}).
